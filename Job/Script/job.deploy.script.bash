...
rsync -rlp -e "ssh -o StrictHostKeyChecking=no -p $SSH_PORT" --delete --exclude-from=./typo3.txt --exclude-from=./local-deploy-rsync-excludes.txt --exclude=/Tests --exclude=auth.json --exclude=local-deploy-rsync-excludes.txt --exclude=codeception.yml .Build/ $SSH_LOGIN:$REMOTE_PATH
